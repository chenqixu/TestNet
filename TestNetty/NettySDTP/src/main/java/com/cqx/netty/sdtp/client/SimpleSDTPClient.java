...
public class SimpleSDTPClient {
    ...
    public String xdr(String header, String rule, String data) throws IOException {
        ...
        try (SocketClient socketClient = SocketClient.newbuilder()
            ...
            // 版本协商
            logger.info("======版本协商开始======");
            result.append("======版本协商开始======").append(SdtpUtil.LineENd);
            ...
            logger.info("======版本协商结束======");
            result.append("======版本协商结束======").append(SdtpUtil.LineENd);

            // 链路鉴权
            logger.info("======链路鉴权开始======");
            result.append("======链路鉴权开始======").append(SdtpUtil.LineENd);
            ...
            socketClient.receive(clientReceive);
            logger.info("======链路鉴权结束======");
            result.append("======链路鉴权结束======").append(SdtpUtil.LineENd);

            // XDR数据发送
            logger.info("======XDR数据发送开始======");
            result.append("======XDR数据发送开始======").append(SdtpUtil.LineENd);
            MessageUtil<SDTPnotifyXDRData_Req> notifyXDRData = new MessageUtil<>(SDTPnotifyXDRData_Req.class, rule);
            for (int i = 0; i < 40; i++) {
                notifyXDRData.append(data);
            }
            ...
            result.append("======XDR数据发送结束======").append(SdtpUtil.LineENd);

            // 链路检测
            logger.info("======链路检测开始======");
            result.append("======链路检测开始======").append(SdtpUtil.LineENd);
            ...
            socketClient.receive(clientReceive);
            logger.info("======链路检测结束======");
            result.append("======链路检测结束======").append(SdtpUtil.LineENd);

            // 链路数据发送校验
            logger.info("======链路数据发送校验开始======");
            result.append("======链路数据发送校验开始======").append(SdtpUtil.LineENd);
            ...
            result.append("======链路数据发送校验结束======").append(SdtpUtil.LineENd);

            // 链路释放
            logger.info("======链路释放开始======");
            result.append("======链路释放开始======").append(SdtpUtil.LineENd);
            ...
            socketClient.receive(clientReceive);
            logger.info("======链路释放结束======");
            result.append("======链路释放结束======").append(SdtpUtil.LineENd);
        }
    ...
    class ClientReceive implements SocketClient.ReceiveCall {
        ...
        public byte[] read(InputStream in) throws IOException {
            ...
            result.append(String.format("[SDTP包体] 长度: %s, [hexStr] %s", lastLen, ByteUtil.bytesToHexStringH(sdtpBodyBytes))).append(SdtpUtil.LineENd);
            switch (msgType) {
                // 1:版本协商应答
                case verNego_Resp:
                    ...
                    result.append("版本协商应答: ").append(verNego_Resp.getResp().getResult()).append(SdtpUtil.LineENd);
                    break;
                // 2:链路鉴权应答
                case linkAuth_Resp:
                    ...
                    result.append(String.format("链路鉴权应答, Result: %s, Digest: %s"
                            , linkAuth_Resp.getResp().getResult(), linkAuth_Resp.getResp().getDigest())).append(SdtpUtil.LineENd);
                    break;
                // 4:xdr数据发送应答
                case notifyXDRData_Resp:
                    ...
                    result.append("xdr数据发送应答: ").append(notifyXdrData_Resp.getResp().getResult()).append(SdtpUtil.LineENd);
                    break;
                // 5:链路检测应答
                case linkCheck_Resp:
                    logger.info("链路检测应答: {}", bodyReadLen);
                    result.append("链路检测应答: ").append(bodyReadLen).append(SdtpUtil.LineENd);
                    break;
                // 6:链路数据发送校验应答
                case linkDataCheck_Resp:
                    ...
                    result.append(String.format("链路数据发送校验应答, Sendflag: %s, Result: %s, SendDataInfo: %s, RecDataInfo: %s"
                            ...
                            , linkDataCheck_Resp.getResp().getResult()
                            , linkDataCheck_Resp.getResp().getSendDataInfo()
                            , linkDataCheck_Resp.getResp().getRecDataInfo())).append(SdtpUtil.LineENd);
                    break;
                // 7:链路释放应答
                case linkRel_Resp:
                    ...
                    result.append("链路释放应答: ").append(linkRel_Resp.getResp().getResult()).append(SdtpUtil.LineENd);
                    break;
                default:
                    break;
            }
