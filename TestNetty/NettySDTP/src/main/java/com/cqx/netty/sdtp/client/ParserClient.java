...
public class ParserClient {
    ...
    private int parser_parallel_num;

    public ParserClient(int seq) {
        this(seq, -1);
    }

    public ParserClient(int seq, int queueLimit) {
        this.ruleUtil = new RuleUtil();
        this.ruleBeanList = ruleUtil.generateRule(httpRule);
        this.before = MetricUtils.getMeter("before" + seq);
        this.after = MetricUtils.getMeter("after" + seq);
        this.cnt = MetricUtils.getMeter("cnt" + seq);
        this.queuePutCnt = MetricUtils.getMeter("queuePutCnt" + seq);
        if (queueLimit > 0) {
    ...
    public static void main(String[] args) {
        ...
        int parallel_num = Integer.valueOf(args[1]);
        logger.info("并发个数：{}", parallel_num);
        int parser_parallel_num = Integer.valueOf(args[2]);
        boolean isSingle = Boolean.valueOf(args[3]);
        logger.info("并发解析还是单线程解析：{}", isSingle ? "单线程解析" : "并发解析");
        if (isSingle) {
            logger.info("执行次数：{}", execCnt);
        } else {
            logger.info("解析并发个数：{}", parser_parallel_num);
            logger.info("队列大小限制：{}", execCnt);
        }
        ...
        if (parallel_num == 1) {
            if (isSingle) {
                ...
                parserClient.setParser_parallel_num(parser_parallel_num);
                parserClient.setExecCnt(execCnt);
                parserClient.parser();
            } else {
                ParserClient parserClient = new ParserClient(seq.getAndIncrement(), execCnt);
                parserClient.setParser_parallel_num(parser_parallel_num);
                parserClient.setExecCnt(execCnt);
                parserClient.startGenerateDataTask();
                parserClient.startParallelParserTask();
            }
        } else {
    ...
    public void setParser_parallel_num(int parser_parallel_num) {
        this.parser_parallel_num = parser_parallel_num;
    }
    ...
    private void startParallelParserTask() {
        ThreadTool threadTool = new ThreadTool(parser_parallel_num);
        for (int i = 0; i < parser_parallel_num; i++) {
            threadTool.addTask(new Runnable() {
                @Override
                public void run() {
                    while (true) {
